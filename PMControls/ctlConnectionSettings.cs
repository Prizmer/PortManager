...
namespace PMControls
{
    public partial class ctlConnectionSettings: UserControl
    {
        ...
        private bool setVirtualPort()
        {
            ...
            try
            {
                if (!this._isTCPSelected)
                {
                    if (!updateComPortSettingFromGUI(ref _comSettings))
                    {
                        ...
                        //MessageBox.Show("", "Порт не создан", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return false;
                    }
                ...
                else
                {
                    if (!updateTCPSettingFromGUI(ref _tcpSettings))
                    {
                        ...
                        updateStatus(msg, true);
                        //MessageBox.Show("Некорректные свойства TCP/IP.", "Порт не создан", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return false;
                    }
            ...
            {
                logger.LogError("Ошибка создания виртуального порта: " + ex.Message);
                return false;
            }
        }
